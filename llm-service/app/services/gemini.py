...
# ── Venue matching removed ──────────────────────────────────────────────────
# Venue identity is now resolved to an integer venue_id at ingestion time by the
# scraper's venue_resolver.py (pipeline stage 2). The LLM no longer matches
# venues against a canonical list, so the Supabase venue-name cache is gone.
...
async def normalize_events(raw_content: str, source_url: str, content_type: str = "html") -> List[Dict]:
    ...
    # Venue matching removed — the scraper assigns venue_id downstream (stage 2).
    ...
    - title (string)
    - venue (string — the venue name exactly as it appears in the source; do NOT canonicalize, the pipeline resolves it to a venue_id downstream)
    ...
    - description (string). Rules:
        1. Generate a clean, engaging summary (2-4 sentences, up to ~500 characters) from the available text and event attributes. Do not cut off important information.
