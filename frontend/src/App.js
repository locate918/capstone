 ...
 * - activeTab: 'thisWeek' | 'allEvents' for filtering
 */

import React, { useState, useEffect, useLayoutEffect } from 'react';
import { Sparkles, Loader2, Map as MapIcon, List, Compass, ChevronLeft, ChevronRight, Calendar, CalendarDays, CalendarRange, X } from 'lucide-react';
...
export default function App() {
    ...
    const [activeTab, setActiveTab] = useState('thisWeek'); // 'thisWeek' | 'allEvents'
    const [startDate, setStartDate] = useState(null); // Date range filter - start
    const [endDate, setEndDate] = useState(null); // Date range filter - end
    ...
    // Scroll to top when search query changes (not on page change)
    useLayoutEffect(() => {
    ...
    // Filter events based on query and active tab
    const filteredEvents = events.filter(e => {
        ...
        if (startDate || endDate) {
            ...
            const eventDateOnly = e.date_iso.split('T')[0];
            
            if (startDate && eventDateOnly < startDate) return false;
            if (endDate && eventDateOnly > endDate) return false;
            
            return true;
        }

        // Then apply tab filter (only for "This Week" tab)
        ...
        // "All Events" tab - show everything (that matches search if applicable)
        return true;
    });
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        {!query && (
                            <div className="flex flex-wrap items-center gap-3">
                                <div className="flex bg-white rounded-lg p-1 border border-slate-200 shadow-sm w-fit">
                                    <button
                                        onClick={() => { setActiveTab('thisWeek'); clearDateRange(); }}
                                        className={`flex items-center gap-2 px-4 py-2 rounded-md text-xs font-bold transition-all ${
                                            activeTab === 'thisWeek' && !startDate && !endDate
                                    ...
                                    <button
                                        onClick={() => { setActiveTab('allEvents'); clearDateRange(); }}
                                        className={`flex items-center gap-2 px-4 py-2 rounded-md text-xs font-bold transition-all ${
                                            activeTab === 'allEvents' && !startDate && !endDate
                                                ? 'bg-slate-900 text-white shadow-md'
                                                : 'text-slate-400 hover:text-slate-900'
                                        }`}
                                    >
                                ...
                                <div className="relative flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-3 py-1.5 shadow-sm">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            if (e.target.value) setActiveTab('allEvents');
                                        }}
                                        className="bg-transparent text-xs font-medium text-slate-600 focus:outline-none cursor-pointer w-[110px]"
                                        placeholder="From"
                                    />
                                    ...
                                    {(startDate || endDate) && (
                                        <button
                                            ...
                                            className="ml-1 bg-slate-900 text-white rounded-full w-5 h-5 flex items-center justify-center hover:bg-red-500 transition-colors"
                                        >
                                            <X size={12} />
                                        </button>
                                    )}
                        ...
                        <h2 className="text-3xl font-serif tracking-tight text-slate-900">
                            {query
                                ...
                                : (startDate || endDate)
                                    ? `Events: ${startDate || 'Any'} ? ${endDate || 'Any'}`
                                    : activeTab === 'thisWeek'
                                        ? 'This Week in Tulsa'
                                        : 'All Events'}
                        </h2>
                        <span className="text-xs text-slate-500 font-medium tracking-wide uppercase">
                            {totalItems} Experiences Found
                        </span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
                        ...
                        <div className={`${viewMode === 'list' ? 'block' : 'hidden lg:block'}`}>
                            {paginatedEvents.length > 0 ? (
                                <>
                                    ...
                                    {/* Pagination Controls */}
                                    {totalPages > 1 && (
                                        <div className="flex justify-center items-center gap-4 mt-16 animate-fade-up">
                                            <button
                                                onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                                                disabled={currentPage === 1}
                                                className="flex items-center justify-center w-10 h-10 rounded-full border border-slate-200 bg-white text-slate-600 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-50 hover:border-[#d4af37] hover:text-[#d4af37] transition-all shadow-sm"
                                ...
                                <div className="flex flex-col items-center justify-center py-24 bg-white/40 rounded-[2rem] border border-dashed border-slate-300 text-center">
                                    ...
                                    <h3 className="text-slate-900 font-serif text-2xl mb-2">
                                        {query ? 'No experiences found' : activeTab === 'thisWeek' ? 'No events this week' : 'No events found'}
                                    </h3>
                                    <p className="text-slate-500 mb-6">
                                        {query
                                            ? 'Try adjusting your search results.'
                                            : activeTab === 'thisWeek'
                                                ? 'Check back later for updates or view all events.'
                        ...
                        <div className={`${viewMode === 'map' ? 'block' : 'hidden lg:block'}`}>
                            <div className="lg:sticky lg:top-[260px] animate-fade-up delay-200">
                                <TulsaMap
                                    events={filteredEvents} // Map shows ALL matching events, not just current page
