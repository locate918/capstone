 ...
 * - Floating legend
 * - Marker highlighting on card hover
 */
...
// Highlight color for hovered markers
...
const injectMapStyles = () => {
  ...
  style.innerHTML = `
    ...
    .marker-highlighted svg {
      animation: pulse-highlight 0.6s infinite ease-in-out;
...
const createPinIcon = (color, isExclusive = false, isHighlighted = false) => {
  let className = 'bg-transparent border-none';
  if (isHighlighted) {
    className += ' marker-highlighted';
  } else if (isExclusive) {
    className += ' marker-exclusive';
  }
...
const getCategoryIcon = (tags, isHighlighted = false) => {
  // If highlighted, use highlight color
  if (isHighlighted) {
    return createPinIcon(HIGHLIGHT_COLOR, false, true);
  }
        ...
        {events.map((event) => {
          ...
          return event.coordinates && (
            <Marker 
              ...
              icon={getCategoryIcon(event.vibe_tags, isHovered)}
            ...
            >
              ...
              <Tooltip 
                ...
                offset={[0, -45]} 
                opacity={1}
                permanent={isHovered} // Keep tooltip open when card is hovered
              >
