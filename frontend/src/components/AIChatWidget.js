...
const AIChatWidget = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { 
          ...
          events: response.events // Store events in message for carousel display
      }]);
    } catch (error) {
  ...
  return (
    <div className="w-full bg-white/60 backdrop-blur-sm rounded-2xl overflow-hidden mb-16 border border-white/60 shadow-2xl animate-fade-up">
      ...
      {/* Messages Area */}
      <div className="h-[400px] overflow-y-auto p-6 bg-white/40 space-y-8">
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
            {/* Text Bubble */}
            <div className={`max-w-[80%] p-4 rounded-2xl shadow-sm text-sm leading-relaxed mb-3 ${
              msg.role === 'user' 
            ...
            {/* Event Cards Carousel (only for assistant messages with events) */}
            {msg.events && msg.events.length > 0 && (
                <div className="w-full overflow-x-auto pb-4 -mx-6 px-6 flex gap-4 snap-x">
                    {msg.events.map((event, i) => (
                        <div key={event.id} className="min-w-[280px] w-[280px] snap-center">
                            <EventCard event={event} index={i} onClick={() => console.log('Clicked event', event.id)} />
                        </div>
                    ))}
                </div>
            )}
